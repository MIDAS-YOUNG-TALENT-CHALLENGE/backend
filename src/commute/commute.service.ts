...
export class CommuteService {
  constructor(
    ...
    private commuteRepository: Repository<CommuteEntity>,
    @InjectRepository(WorkingHourEntity)
    private workingHourRepository: Repository<WorkingHourEntity>,
  ...
  async GetAllCommute(dto: GetAllCommuteDTO) {
    ...
    if (commutes.length == 0)
      throw new NotFoundException('출퇴근 정보를 찾을 수 없습니다');
    return commutes;
  }
  ...
  async GetCommuteState(userId: number) {
    ...
    if (workingWeeklyHour.length === 0)
      throw new NotFoundException('총 근무 시간 정보를 찾을 수 없습니다.');

    const newYearDate = commutes[0].date.getFullYear().toString() + '-01-01';

    // TODO::출퇴근이 다른 일자일 때도 체크
    const workingAttendenceHourSum = await this.commuteRepository
      .createQueryBuilder()
      .where('week = :week', { week: commutes[0].week })
      .andWhere(':newYearDate <= date AND date <= :todayDate', {
        newYearDate: newYearDate,
        todayDate: commutes[0].date,
      })
      .andWhere('state = :state', { state: CommuteState.ATTENDANCE })
      .select('SUM(date)', 'sum')
      .getRawOne();

    const workingLeavingHourSum = await this.commuteRepository
      .createQueryBuilder()
      .where('week = :week', { week: commutes[0].week })
      .andWhere(':newYearDate <= date AND date <= :todayDate', {
        newYearDate: newYearDate,
    ...
    const workingWeeklyTotalHour = workingLeavingHourSum.sum
      ? workingLeavingHourSum.sum - workingAttendenceHourSum.sum
      : 0;

    if (commutes[0].state === CommuteState.LEAVE) {
      // 퇴근시간 - 출근시간
      const workingHour =
        commutes[0].date.getTime() - commutes[1].date.getTime();
      const todayState =
        workingHour / (1000 * 60 * 60) - workingWeeklyHour[0].workingHour / 5;
      const weeklyState =
        workingWeeklyHour[0].workingHour - workingWeeklyTotalHour / 10000;
      return {
    ...
    if (commutes[0].state === CommuteState.ATTENDANCE) {
      ...
      const timeFromAttendanceHour = Math.floor(
        timeFromAttendance / (1000 * 60 * 60),
      );
      const timeFromAttendanceMinute =
        Math.floor(timeFromAttendance / (1000 * 60)) -
        timeFromAttendanceHour * 60;
      const weeklyState =
        workingWeeklyHour[0].workingHour - workingWeeklyTotalHour / 10000;
      return {
